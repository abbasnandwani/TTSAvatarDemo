...
var lastSpeakTime
var imgUrl = ""
var chatEndpoint = "https://localhost:7058/Chat/PostStream" //replace with DemoKBApi endpoint

// Connect to avatar service
function connectAvatar() {
    const cogSvcRegion = "eastus2" // document.getElementById('region').value
    const cogSvcSubKey = "test" //document.getElementById('APIKey').value
    if (cogSvcSubKey === '') {
    ...
    const talkingAvatarCharacter = "lisa" //document.getElementById('talkingAvatarCharacter').value
    const talkingAvatarStyle = "casual-sitting" //document.getElementById('talkingAvatarStyle').value
    ...
    const azureOpenAIEndpoint = chatEndpoint //document.getElementById('azureOpenAIEndpoint').value
    const azureOpenAIApiKey = "test" //document.getElementById('azureOpenAIApiKey').value
    const azureOpenAIDeploymentName = "test" //document.getElementById('azureOpenAIDeploymentName').value
...
function handleUserQuery(userQuery, userQueryHTML, imgUrlPath) {
    ...
    if (dataSources.length > 0 || enableQuickReply) { //if (dataSources.length > 0 && enableQuickReply) {
    ...
    let url = "{AOAIEndpoint}".replace("{AOAIEndpoint}", azureOpenAIEndpoint).replace("{AOAIDeployment}", azureOpenAIDeploymentName)
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            let userMsg = document.getElementById('userMessageBox')
            ...
            function read(previousChunkString = '') {
                return reader.read().then(({ value, done }) => {
                    // Check if there is still data to read test
                    ...
                    chunkString.split('\n\n').forEach((line) => {
                        try {
                            ...
                            if (line.startsWith('data:') && !line.endsWith('[DONE]')) {
                                ...
                                let responseNotSpoken = undefined
                                if (dataSources.length === 0) {
                                    //responseToken = responseJson.choices[0].delta.content
                                    responseToken = responseJson.Content
                                    if (responseJson.NotSpokenContent != null) {
                                        responseNotSpoken = responseJson.NotSpokenContent
                                        //responseNotSpoken.replace(/\n/g, '<br/>')
                                        userMsg.innerHTML += md.render(responseNotSpoken);
                                        console.log("Abbas1: " + responseNotSpoken);
                                    }
                                } else {
                                    let role = responseJson.Role
                                    if (role === 'tool') {
                                        toolContent = responseJson.Content
                                    } else {
                                        responseToken = responseJson.Content
